...
class RasterEquivalentLayerDialog(QtWidgets.QDialog, Ui_AGTRasterEquivalentLayerDialog):
    ...
    def inputCheck(self):
        ...
        if not self.outputFilename.text():
            msg = QCoreApplication.translate(u"RasterEquivalentLayerDialog", 'Please specify an output filename.')            
            QtWidgets.QMessageBox.warning(self, 'AGT', msg)
            return False
        isAscii = lambda s: len(s) == len(s.encode())
        if not isAscii(os.path.basename(self.outputFilename.text())):
            msg = QCoreApplication.translate(u"RasterEquivalentLayerDialog", 'The output filename should only have ASCII characters.')            
            QtWidgets.QMessageBox.warning(self, 'AGT', msg)
            return False
